        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 font-medium mt-1">Map strategic goals to actual shipped features.</p>
            </div>
          </div>
        </div>
        <button
          onClick={() => setShowAddForm(true)}
          className="flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-semibold shadow-lg shadow-orange-500/20 transition"
        >
          <Plus size={16} />
          New Objective
        </button>
      ...
      {showAddForm && (
        <div className="mb-8 bg-[#171923] border border-white/[0.08] p-6 rounded-2xl max-w-2xl">
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Objective Title</label>
              <input
                type="text"
                value={newTitle}
                onChange={(e) => setNewTitle(e.target.value)}
                placeholder="e.g. Expand into Enterprise Market"
                className="w-full bg-white/[0.04] border border-white/[0.08] text-white text-sm px-4 py-2.5 rounded-lg focus:outline-none focus:border-orange-500/50 transition"
                autoFocus
                required
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Target Metric</label>
              <input
                type="text"
                value={newTarget}
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                type="submit"
                className="px-6 py-2 bg-orange-600 hover:bg-orange-500 text-white text-sm font-semibold rounded-lg transition"
              >
                Save OKR
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {objectives.length === 0 && !showAddForm && (
          <div className="col-span-full py-20 flex flex-col items-center justify-center border-2 border-dashed border-white/[0.05] rounded-3xl">
            ...
            <p className="text-gray-500 mt-2 text-center max-w-sm">Strategic goals are the backbone of a great product. Define high-level objectives here and link features to them on the board.</p>
          </div>
        )}
        ...
        {objectives.map((okr) => {
          ...
          return (
            <div key={okr.id} className="bg-[#171923] border border-white/[0.08] rounded-2xl p-6 group hover:border-white/[0.15] transition flex flex-col">
              <div className="flex justify-between items-start mb-4">
                <div className="px-2 py-1 bg-white/[0.05] text-gray-400 text-[10px] font-bold uppercase tracking-wider rounded border border-white/[0.1]">
                  Objective
                </div>
                <button 
                  onClick={() => deleteObjective(okr.id)}
                  className="p-1.5 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition rounded hover:bg-white/[0.05]"
                >
                  <Trash2 size={16} />
                </button>
              </div>
              
              <h3 className="text-xl font-bold text-white mb-2 leading-tight">{okr.title}</h3>
              <p className="text-sm text-gray-400 mb-6 flex-1">{okr.targetMetric}</p>
              ...
              <div className="mt-auto pt-6 border-t border-white/[0.06]">
                <div className="flex justify-between items-end mb-3">
                  <div>
                    ...
                    <span className="text-xs text-gray-500 ml-2 font-medium">Progress</span>
                  </div>
