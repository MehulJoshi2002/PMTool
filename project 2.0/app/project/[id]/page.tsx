...
export default function LeaderboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0E1015] p-8 pb-32">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <Link href="/" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-white transition mb-8">
        ...
        {/* Global List */}
        <div className="bg-[#171923] rounded-xl border border-white/[0.06] shadow-xl overflow-hidden">
          <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/[0.06] text-xs font-bold text-gray-500 uppercase tracking-widest bg-black/20">
            ...
            {scoredFeatures.map((feature, idx) => (
              <div key={feature.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-white/[0.02] transition">
                <div className="col-span-1 text-center">
                  <span className={`text-lg font-black font-mono ${idx === 0 ? 'text-yellow-500' : idx === 1 ? 'text-gray-300' : idx === 2 ? 'text-amber-700' : 'text-gray-600'}`}>
                    #{idx + 1}
                  </span>
                </div>
                <div className="col-span-1">
                  <span className="text-[10px] font-mono font-bold text-gray-500">{feature.productId}</span>
                </div>
                ...
                <div className="col-span-2 flex flex-wrap gap-1">
                  ...
                  {(feature.labels || []).length > 2 && (
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-gray-500/10 text-gray-400 border border-gray-500/20">
                      +{feature.labels!.length - 2}
                    </span>
                  )}
                </div>
        ...
        {unscoredFeatures.length > 0 && (
          <div className="mt-12">
            <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4 ml-6">Unscored Backlog ({unscoredFeatures.length})</h2>
            <div className="bg-[#171923] rounded-xl border border-dashed border-white/[0.06] overflow-hidden opacity-70 hover:opacity-100 transition">
              {unscoredFeatures.map((feature) => (
                <div key={feature.id} className="flex items-center gap-4 px-6 py-3 border-b border-white/[0.04]">
                  <span className="text-[10px] font-mono font-bold text-gray-600 w-12">{feature.productId}</span>
